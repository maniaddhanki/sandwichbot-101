...
uint16_t RCSensor::readLinePrivate(uint16_t sensorValues[], bool invertReadings)
{
  bool onLine = false;
...
int RCSensor::isSensorOnline(uint16_t sensorValues[], int threshold, int index)
{
...
void RCSensor::loadCalibratedLineSensors(uint16_t lineSensorMaxCalib[5], uint16_t lineSensorMinCalib[5])
{
...
void RCSensor::loadCalibratedSideSensors(uint16_t sideSensorMaxCalib[6], uint16_t sideSensorMinCalib[6])
{
